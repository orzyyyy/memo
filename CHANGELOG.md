...
- document
  ...
  - 从 `react-markdown` 迁移到 `marked` #204
    - 加入 `github-markdown-css`
  - 增加 `visible` 属性以控制 MainPageList 项的显隐 #212
  - 增加隐藏 MainPageList 项的接口 #214
    - 位于右键菜单中
  - 设置 `mini-xmind` 默认方向为横向 d26d5269cd15205960ac6b8c1993d06295dd9948
...
- exhentai
  ...
  - 重写 exhentai 中 的`/download/status` #233
    ...
    - 现在不需要再传 `name`
    - `dateStamp` 可选
  - 检查图片状态时返回漫画名称 cdbe9b18559f67f3a9bfa7c43816bfe78de8fb7c

- 构建

  - 从 CircleCI 迁移到 github actions #201
  - 线上构建时，不再上传 mapping.json 中 `visible` 为 false 的文件 #213
  - 移除 `moment` 的 locales 866b200c49f634816c99a92a388b1250db4991fa
  - 构建时为每个路由创建 html 模板 #229 #230
    - 从 HashRouter 迁移到 BrowserRouter
    - 同时支持 netlify 和 gh-pages 3db201b56cab1c5960200b987b55f3eb46a884d4
  - 线上环境压缩 json 文件 #243
  - 现在只会在 release 时 deploy 到 gh-pages，netlify 仍然是合并到 master 时构建 04b91b3b39c2f8dbfba0753623a617056fb34f83

- 其他
  - 更新 banner #215
  - 增加通用接口查询 mysql #225 424fa5877307f23267dc9ab1022193089e685ec6 6ea552fe6f2615c461cca2f28ceee5231477efda
